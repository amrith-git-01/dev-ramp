...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="RepoRadar - AI-Powered Legacy Codebase Onboarding Accelerator",
...
Examples:
  ...
  python run_analysis.py --verbose
    ...
    parser.add_argument(
        "--mcp-config", type=str, help="Path to MCP servers configuration JSON file"
    )
...
def print_summary(results: dict):
    ...
    if agent_results:
        ...
        for agent_name, agent_result in agent_results.items():
            ...
            print(
                f"{status_icon} {agent_name.capitalize()}: {agent_status} ({elapsed:.2f}s)"
            )
