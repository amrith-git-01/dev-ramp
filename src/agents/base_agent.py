"""
Base Agent Class for RepoRadar AI Agents
...
class BaseAgent(ABC):
    """
    Abstract base class for all RepoRadar AI agents.
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                response = await loop.run_in_executor(
                    None, lambda: self.model.generate(prompt=prompt, params=params)
                )
    ...
    def truncate_text(
        self, text: str, max_length: int = 1000, suffix: str = "..."
    ) -> str:
        """
    ...
    async def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"{self.name} agent completed in {elapsed_time:.2f}s")
