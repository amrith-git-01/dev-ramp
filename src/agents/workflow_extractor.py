...
class WorkflowExtractor(BaseAgent):
    ...
    async def analyze(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if workflow_data is None:
            workflow_report = await self._generate_workflow_guide(
                workflows, json_only=True
            )
            workflow_data = self._parse_workflow_json(workflow_report)
        if workflow_data is None:
            workflow_data = build_fallback_workflow_diagram(
        ...
        for wf in workflow_data.get("workflows", []):
            ...
            if seq:
                slug = re.sub(
                    r"[^a-z0-9]+", "-", wf.get("name", "workflow").lower()
                ).strip("-")
                write_mermaid_file(docs_root, f"workflow-{slug}", seq)
                lines.append("\n")
                lines.append(
                    embed_mermaid_in_markdown("Sequence", seq, heading_level=3)
                )
            lines.append("\n")
    ...
    async def _generate_workflow_guide(
        ...
        json_suffix = (
            " Return ONLY raw JSON, no markdown fences or prose." if json_only else ""
        )
...
**Generated by:** RepoRadar Workflow Extractor
    ...
    def _extract_setup_info(self, workflows: Dict[str, Dict]) -> str:
        ...
        if "package.json" in workflows:
            try:
                ...
                if "engines" in pkg:
                    info_parts.append(
                        f"- Node Version: {pkg['engines'].get('node', 'not specified')}"
                    )
                if "scripts" in pkg:
                    info_parts.append(
                        f"- Available Scripts: {', '.join(list(pkg['scripts'].keys())[:5])}"
                    )
            except:
                pass
        ...
        return (
            "\n".join(info_parts)
            if info_parts
            else "No specific setup information found in workflow files."
        )
